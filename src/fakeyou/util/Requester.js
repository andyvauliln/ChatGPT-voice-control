...
class Requester extends null {
    static async __getData(url, headers) {
        // console.log(url, headers, 'get_data');
        try {
    ...
    static async __postData(url, data, headers) {
        console.log(url, data, headers, 'post_data');
    ...
    static async __login(client) {
        ...
        try {
            console.log('Login', Constants.URL.login, options);
            ...
            if (body.success == true) {
                if (!headers["set-cookie"]) {
                    client.session.auth = "eyJhbGciOiJIUzI1NiJ9.eyJjb29raWVfdmVyc2lvbiI6IjIiLCJzZXNzaW9uX3Rva2VuIjoiU0VTU0lPTjo3d3ExcmUzdnY4cjduOTM0Z2Zmd3FiOXgiLCJ1c2VyX3Rva2VuIjoiVToyRVlRS1dBSzBQTVk0In0.VmCMwM1DdK5M0Kwt7yLo6PkOmA3xns_3ruqEj12Ik_c"
                }
                else {
    ...
    static async __session(client) {
        ...
        console.log(client.session.auth, 'client.session.auth');
        ...
        console.log(logged_in, user, 'client.session.auth');
